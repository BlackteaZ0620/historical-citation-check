...
本 skill 用于帮助历史学方向的论文写作者，检查脚注、尾注或参考注释是否符合较规范的历史研究注释体例。

它的核心任务不是直接替用户改写 Word 论文，而是识别格式问题，并生成一份可阅读的 Markdown 错误报告。遇到外文书名、期刊名时，应识别并保留斜体信息，在 Markdown 中使用斜体标记，例如 `*Modern China*`；遇到中文文献题名时，应保留书名号《》；遇到文章篇名、析出文献、档案文献、古籍版本等情况时，应按照下列规则处理。
...
1. 先判断每条注释属于哪一种文献类型：著作、译著、析出文献、序言、古籍、期刊、报纸、档案、外文文献、再次引证、间接引文或夹注。同一文献再次引证时，通常只需保留责任者、题名和页码，出版地、出版社、年份等出版信息可以省略。按照相应类型的“标注顺序”检查字段。必须先识别各模块的结构边界，再判断格式问题。外文注释的基本结构为：责任者, *书名*, 出版地: 出版社, 出版年, p. X。书名结束于出版地之前。书名内部出现的年份范围（如 1851-1939、1876-1909）通常属于书名的一部分，不应误判为出版年份。出版年份总是单一年份，出现在出版社之后。
2. 指出标点问题时，必须写清楚是哪一个标点，例如“`Minawi,` 后缺少空格”或“`London :` 中冒号前多出空格”。英文出版地后应使用英文冒号 `:`”。中文注释中的逗号、冒号、书名号和句号应与示例一致；英文注释中的逗号、卷号、期号、出版地、出版社、年份和页码标识应与示例一致。
...
12. 中文著作中，责任方式为"著"时"著"可省略，责任者后直接用冒号连接题名（如 实藤惠秀：《中国人留学日本史》）完全合规，不要将省略"著"标记为错误或建议补加"著"字。只有当其他责任方式（如"编""主编""整理""译"）被省略时，才应标记为缺项。引用翻译著作时，译者应作为第二责任者置于文献题名之后（如 实藤惠秀：《中国人留学日本史》，谭汝谦、林启彦译，香港：……）。如果译者被放在责任者之后、题名之前（如 伯纳德·刘易斯著，范中廉译：《现代土耳其的兴起》），应标记为译者位置错误，建议移至题名之后。
13. 保留原文已有的有效信息，不要凭空编造作者、出版社、出版年、页码、卷册号或馆藏信息。
14. 如果出版时间缺失，按照规范写作 `[出版时间不详]`；如果其他必要信息缺失，应明确提示用户“缺某项信息”，不要假装已经补齐。
15. 输出 `citation-errors.md`，并在对话中简要说明文件位置和主要发现。只有在用户明确要求 JSON 辅助数据时，才另行生成按文献类型定制的 JSON 文件。
...
1. 如果注释只写作者、短题名和页码，应先判断它是否可能是再次引证；只有在无法对应前文完整引证时，才提示“需确认前文是否已有完整引证”。
2. 不同文献类型的必要字段不同。不要要求档案文献必须有出版社，也不要要求议会文书、古籍、未刊手稿一定具备期刊卷号或期号。
3. 对疑似问题使用“疑似”“需复核”“建议确认”这样的措辞；对确定问题使用“缺少”“多出”“应改为”这样的措辞。
